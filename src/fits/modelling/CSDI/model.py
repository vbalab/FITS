...
class CSDI_base(nn.Module):
    def __init__(self, target_dim, config, device):
        ...
        # decides whether the diffusion model gets any conditioning context (observed parts) or whether it must generate everything from noise:
    ...
    def calc_loss_valid(
        self, observed_data, cond_mask, observed_mask, cond_info, is_train
    ):
        loss_sum = 0

        for t in range(self.num_steps):  # calculate loss for all t
            loss = self.calc_loss(
                observed_data, cond_mask, observed_mask, cond_info, is_train, set_t=t
            )
    ...
    def forward(self, batch, is_train=1):
        ...
        cond_info = self.get_cond_info(observed_tp, cond_mask)
    ...
    def impute(     # reverse diffusion to fill in missing values
        ...
        cond_mask,
        cond_info,
        n_samples,  # number of different imputations (samples) to generate for each input sequence
    ):
        B, K, L = observed_data.shape
    ...
    def evaluate(self, batch, n_samples):
        ...
        with torch.no_grad():
            ...
            cond_info = self.get_cond_info(observed_tp, cond_mask)
            samples = self.impute(observed_data, cond_mask, cond_info, n_samples)
